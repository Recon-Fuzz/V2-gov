# Function: test_voting_power_no_difference_in_allocating_start_or_end_of_epoch()
...
function test_voting_power_no_difference_in_allocating_start_or_end_of_epoch() public {
    ...
    vm.warp((block.timestamp + (EPOCH_DURATION * 2)) - governance.EPOCH_VOTING_CUTOFF());
    _allocateLQTY(user, lqtyAmount);
    vm.warp((block.timestamp + EPOCH_DURATION) + 1);
    governance.snapshotVotesForInitiative(baseInitiative1);
    (uint256 votesFromAllocatingAtEpochEnd, , , ) = governance.votesForInitiativeSnapshot(baseInitiative1);
    assertEq(votesFromAllocatingAtEpochStart, votesFromAllocatingAtEpochEnd, "allocating is more favorable at certain point in epoch");
}
```
...
- **Vm::revertToState(uint256)**
- **GovernanceTester::EPOCH_VOTING_CUTOFF()**
...
```
┌─ [0] ⚙️ FUNCTION: GovernanceTest.test_voting_power_no_difference_in_allocating_start_or_end_of_epoch() (NodeID: 0)
    💬 Args: [no args]
    👁️  Def: public
  ...
  └─ [1] ⚙️ FUNCTION: StdAssertions.assertEq(uint256,uint256,string) (NodeID: 4)
      💬 Args: [votesFromAllocatingAtEpochStart, votesFromAllocatingAtEpochEnd, "allocating is more favorable at certain point in epoch"]
      👁️  Def: internal
```
