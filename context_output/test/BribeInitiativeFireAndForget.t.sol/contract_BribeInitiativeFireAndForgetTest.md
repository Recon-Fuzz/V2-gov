...
- **Name**: BribeInitiativeFireAndForgetTest
- **Type**: Contract
- **Path**: test/BribeInitiativeFireAndForget.t.sol
...
```solidity
uint32 internal constant EPOCH_VOTING_CUTOFF = 6 days
```

### MAX_NUM_EPOCHS

```solidity
uint256 internal constant MAX_NUM_EPOCHS = 100
```

### MAX_VOTE

```solidity
uint256 internal constant MAX_VOTE = 1e6 ether
```

### MAX_BRIBE

```solidity
uint128 internal constant MAX_BRIBE = 1e6 ether
```

### MAX_CLAIMS_PER_CALL

```solidity
uint256 internal constant MAX_CLAIMS_PER_CALL = 10
```

### MEAN_TIME_BETWEEN_VOTES

```solidity
uint256 internal constant MEAN_TIME_BETWEEN_VOTES = 2 * EPOCH_DURATION
```

### VOTER_PROBABILITY

```solidity
uint256 internal constant VOTER_PROBABILITY = type(uint256).max / 10
```
...
```solidity
address internal constant other = address(uint160(uint256(keccak256("other"))))
```

### briber

```solidity
address internal constant briber = address(uint160(uint256(keccak256("briber"))))
```

### config

```solidity
IGovernance.Configuration internal config = IGovernance.Configuration({registrationFee: 0, registrationThresholdFactor: 0, unregistrationThresholdFactor: 4 ether, unregistrationAfterEpochs: 4, votingThresholdFactor: 1e4, minClaim: 0, minAccrual: 0, epochStart: START_TIME - EPOCH_DURATION, epochDuration: EPOCH_DURATION, epochVotingCutoff: EPOCH_VOTING_CUTOFF})
```
...
```solidity
mapping(address => address[]) internal initiativesToReset
```

### latestVote

```solidity
mapping(address => Vote) internal latestVote
```

### boldAtEpoch

```solidity
mapping(uint256 => uint256) internal boldAtEpoch
```

### brybAtEpoch

```solidity
mapping(uint256 => uint256) internal brybAtEpoch
```

### voteAtEpoch

```solidity
mapping(uint256 => uint256) internal voteAtEpoch
```

### toteAtEpoch

```solidity
mapping(uint256 => uint256) internal toteAtEpoch
```
...
### test_AbleToClaimBribesInAnyOrder_EvenFromEpochsWhereVoterStayedInactive(bytes32)

- **Signature**: `test_AbleToClaimBribesInAnyOrder_EvenFromEpochsWhereVoterStayedInactive(bytes32)`
- **Visibility**: external
- **Source Range**: 4850:5521:94
- **Details**: [function_test_AbleToClaimBribesInAnyOrder_EvenFromEpochsWhereVoterStayedInactive_bytes32.md](./function_test_AbleToClaimBribesInAnyOrder_EvenFromEpochsWhereVoterStayedInactive_bytes32.md)
