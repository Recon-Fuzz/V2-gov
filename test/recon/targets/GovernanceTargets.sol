...
abstract contract GovernanceTargets is BaseTargetFunctions, Properties {
    ...
    function governance_allocateLQTY_clamped(
        ...
        // Clamp array sizes to maximum of 10
        if (_initiativesToReset.length > 10) {
            assembly {
                mstore(_initiativesToReset, 10)
            }
        }
        if (_initiatives.length > 10) {
            assembly {
                mstore(_initiatives, 10)
            }
        }
        if (_absoluteLQTYVotes.length > 10) {
            assembly {
                mstore(_absoluteLQTYVotes, 10)
            }
        }
        if (_absoluteLQTYVetos.length > 10) {
            assembly {
                mstore(_absoluteLQTYVetos, 10)
            }
        }
        
        // Clamp vote amounts to user's unallocated LQTY
    ...
    function governance_resetAllocations_clamped(
        ...
        governance_resetAllocations(_initiativesToReset, true);
    }
    ...
    function governance_withdrawLQTY_clamped(uint256 _lqtyAmount) public asActor {
        // Clamp amount to user's unallocated LQTY
        (uint256 unallocatedLQTY,,,) = governance.userStates(_getActor());
    ...
    function governance_multiDelegateCall_clamped(
        ...
        // Clamp array size to maximum of 5
        if (_calls.length > 5) {
            assembly {
                mstore(_calls, 5)
            }
