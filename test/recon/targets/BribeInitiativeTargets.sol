    ...
    Properties
{
    ...
    function clamped_bribeInitiative_onAfterAllocateLQTY(uint256 _currentEpoch, address _user, IGovernance.UserState memory _userState, IGovernance.Allocation memory _allocation, IGovernance.InitiativeState memory _initiativeState) public asActor {
        // Apply meaningful values from meaningful-values.json
        _currentEpoch = governance.epoch();
        _user = _getActor();
        (uint256 unallocatedLQTY, uint256 unallocatedOffset, uint256 allocatedLQTY, uint256 allocatedOffset) = governance.userStates(_user);
    ...
    function clamped_bribeInitiative_onClaimForInitiative(uint256 _currentEpoch, uint256 _rewardAmount) public asActor {
        // Apply meaningful values from meaningful-values.json
        _currentEpoch = governance.epoch();
